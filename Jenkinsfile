@Library('khulnasoft-pipeline-lib@master') _
import com.khulnasoft.deployments.orchestrators.*

def orchestrator = new OrcFactory(this).GetOrc()
def charts = ['server', 'kube-enforcer', 'enforcer', 'gateway', 'khulnasoft-quickstart', 'cyber-center', 'cloud-connector', 'scanner', 'tenant-manager', 'codesec-agent']
        ...
        stage("Vul scan") {
            steps {
                script {
                    parallel charts.collectEntries { chart ->
                        ["${chart}": {
                            stage("Vul scan ${chart}") {
                                helmBasic.vulScan(chart)
                            }
                        }]
                    }
                }
    ...
    post {
        always {
            script {
                ...
                cleanWs()
                notifyFullJobDetailes subject: "${env.JOB_NAME} Pipeline | ${currentBuild.result}", emails: 'deployments@khulnasoft.com'
            }
        }
    }
}